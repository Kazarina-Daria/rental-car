...
export default function CarDetailsClient({ car, carId }: CarDetailsClientProps) {
  ...
  const mutation = useMutation<BookingResponse, AxiosError<{ message?: string }>, BookingRequest>({
    mutationFn: (values: BookingRequest) => bookingRequest(carId, values),
    onSuccess: (data) => {
      toast.success(data.message || "Booking request created!");
    },
    onError: (err) => {
      const errorMessage = err.response?.data?.message || err.message || "Invalid request.";
    ...
    <section className={css.sectionId}>
      <div className={css.leftSide}>
        <div className={css.imgBox}>
          <Image src={car.img} alt={`${car.brand} ${car.model}`} width={640} height={512} loading="eager" />
        </div>
      ...
      <div>
        <div className={css.brandBox}>
          ...
          <p className={css.location}>
            ...
            <span className={css.city}>{city}{country ? `, ${country}` : ""}</span>
        ...
        <div className={css.specificationsBox}>
          ...
          <ul className={css.list}>
            ...
            <li className={css.item}>
              ...
              <span className={css.text}>Fuel Consumption: {car.fuelConsumption}</span>
            </li>
            <li className={css.item}>
              <svg width={16} height={16}>
